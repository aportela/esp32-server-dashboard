...
LGFXScreenCPUDetails::LGFXScreenCPUDetails(LovyanGFX *display, SourceData *sourceData) : LGFXScreen(display)
{
    if (sourceData != nullptr)
    {
        ...
        if (display != nullptr)
        {
            ...
            this->cpuLoadBlock4 = new LGFXScreenDashboardResumeEntityCPULoad(display, sourceData, METER_GRAPH_WIDTH, METER_GRAPH_HEIGHT, 0, (METER_GRAPH_HEIGHT + 11) * 3, CPU_USAGE_TYPE_CPU_NICE);
            this->cpuLoadBlock5 = new LGFXScreenDashboardResumeEntityCPULoad(display, sourceData, METER_GRAPH_WIDTH, METER_GRAPH_HEIGHT, 0, (METER_GRAPH_HEIGHT + 11) * 4, CPU_USAGE_TYPE_CPU_IOWAIT);
            this->Refresh(true);
        }
    }
}
